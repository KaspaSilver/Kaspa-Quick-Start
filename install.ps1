...
function Get-Stack {
    ...
    foreach ($item in @('docker-compose.yml', 'kaspad', 'manager', 'bridge', 'kachat', 'nextcloud', 'uninstall.sh', 'uninstall.ps1', 'README.md')) {
    ...
    # kaspad reads this the moment it boots, and on a fresh install that races
    # the manager writing it -- the node would come up on kaspad's own defaults
    # (gRPC on 127.0.0.1, no wRPC at all) and the panel could not reach it.
